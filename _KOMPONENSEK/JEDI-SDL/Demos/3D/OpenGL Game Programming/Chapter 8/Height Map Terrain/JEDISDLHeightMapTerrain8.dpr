program JEDISDLHeightMapTerrain8;
{******************************************************************}
{                                                                  }
...
const
  ...
  WINDOW_TITLE = 'OpenGL Game Programming Chapter 8 : Terrain Demo';
  SCREEN_WIDTH = 640;
  SCREEN_HEIGHT = 480;
  SCREEN_BPP = 16;
...
var
  ...
  cameraX, cameraY, cameraZ : TGLfloat;	// camera coordinates
  lookX, lookY, lookZ : TGLfloat;			// camera look-at coordinates
  ...
  ////// Terrain Data
  terrain : array [0..MAP_X-1] of array [0..MAP_Z-1] of array [0..2] of TGlFloat ;		// heightfield terrain data (0-255); 256x256
...
begin
  ...
  begin
    ...
    begin
      ...
      //terrain[x][z][1] := HeightMap.pixels[ ( z * MAP_Z + x ) * 3 ];
      terrain[x][z][1] := PByteArray(HeightMap.pixels)[( z * MAP_Z + x ) * 3 ];
...
begin
  ...
  cameraY := lookY + mouseY / 2.0;

  // calculate the camera look-at coordinates as the center of the terrain map
  lookX := (MAP_X*MAP_SCALE)/2.0;
  lookY := 150.0;
  lookZ := -(MAP_Z*MAP_SCALE)/2.0;
  ...
  begin
    ...
    begin
      ...
      // draw vertex 0
      glColor3f(terrain[x][z][1]/255.0, terrain[x][z][1]/255.0, terrain[x][z][1]/255.0);
      glTexCoord2f(0.0, 0.0);
      glVertex3f(terrain[x][z][0], terrain[x][z][1], terrain[x][z][2]);

      // draw vertex 1
      glTexCoord2f(1.0, 0.0);
      glColor3f(terrain[x+1][z][1]/255.0, terrain[x+1][z][1]/255.0, terrain[x+1][z][1]/255.0);
      glVertex3f(terrain[x+1][z][0], terrain[x+1][z][1], terrain[x+1][z][2]);

      // draw vertex 2
      glTexCoord2f(0.0, 1.0);
      glColor3f(terrain[x][z+1][1]/255.0, terrain[x][z+1][1]/255.0, terrain[x][z+1][1]/255.0);
      glVertex3f(terrain[x][z+1][0], terrain[x][z+1][1], terrain[x][z+1][2]);

      // draw vertex 3
      glColor3f(terrain[x+1][z+1][1]/255.0, terrain[x+1][z+1][1]/255.0, terrain[x+1][z+1][1]/255.0);
