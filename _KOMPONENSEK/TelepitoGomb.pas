...
type
  ...
  TTelepitoGomb = class(TGraphicControl)
  private
    FAllowTimer:        boolean;
  ...
  published
    ...
    property ColorActive: TColor read fColorActive write SetColorActive default 0;
    ...
    property FontActive: TColor read fFontActive write SetFontActive default 0;
...
{ TTelepitoGomb }
constructor TTelepitoGomb.Create(AOwner: TComponent);
begin
...
destructor  TTelepitoGomb.Destroy;
begin
  FFont.Free;
  FGlyph.Free;
...
procedure TTelepitoGomb.CMDialogChar(var Message: TCMDialogChar);
begin
...
procedure TTelepitoGomb.CMEnabledChanged(var Message:TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TTelepitoGomb.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TTelepitoGomb.CMTextChanged(var msg: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TTelepitoGomb.DrawBorder;
//var i      : integer;
//    Dest,
//    Source : TRect;
begin
...
procedure TTelepitoGomb.DrawGradient;
var
  OutRect,
  TempRect   : TRect;
  TempStepV  : Single;
...
procedure TTelepitoGomb.LoadColors;
var StartColor,
    EndColor       : TColor;
    AContrast,
...
procedure TTelepitoGomb.Loaded;
begin
  inherited Loaded;
  LoadColors;
end;

procedure TTelepitoGomb.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TTelepitoGomb.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  with Canvas do begin
    ...
    if Caption<>'' then begin
      ...
      TextTop:=(CRect.Top+CRect.Bottom-TextHeight(Caption)) div 2;
...
procedure TTelepitoGomb.SetBevelWidth(NewValue: integer);
begin
...
procedure TTelepitoGomb.SetBorderColor(newColor: TColor);
begin
...
procedure TTelepitoGomb.SetBorderStyle(newValue: TBorderStyle);
begin
...
procedure TTelepitoGomb.SetChangeDirection(newValue: boolean);
begin
...
procedure TTelepitoGomb.SetColor(newColor: TColor);
begin
...
procedure TTelepitoGomb.SetContrastHighlight(newValue: TContrast);
begin
...
procedure TTelepitoGomb.SetContrastShadow(newValue: TContrast);
begin
...
procedure TTelepitoGomb.SetDown(newValue: boolean);
begin
...
procedure TTelepitoGomb.SetFont(newFont: TFont);
begin
...
procedure TTelepitoGomb.SetGlyph(newGlyph: TBitmap);
begin
...
procedure TTelepitoGomb.SetGradientDirection(newValue: TGradDirection);
begin
...
procedure TTelepitoGomb.SetGradientStyle(newValue: TGradientStyle);
begin
...
procedure TTelepitoGomb.SetLayout(newValue: TButtonLayout);
begin
...
procedure TTelepitoGomb.SetMargin(newValue: integer);
begin
...
procedure TTelepitoGomb.SetNumGlyphs(newNumGlyphs: TNumGlyphs);
begin
...
procedure TTelepitoGomb.SetSpacing(newValue: integer);
begin
...
procedure TTelepitoGomb.TimerExpired(Sender: TObject);
begin
...
procedure TTelepitoGomb.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  Message.Result:=1;
end;
...
procedure Register;
begin
  RegisterComponents('Md',[TTelepitoGomb]);
end;

procedure TTelepitoGomb.MouseEnter(var Msg: TMessage);
begin
 fOrigFont.Color := FFont.Color;
 fFont.Color := fFontActive;
 fOrigColor := fColor;
 fColor := fColorActive;
 Invalidate;
end;

procedure TTelepitoGomb.MouseLeave(var Msg: TMessage);
begin
 fFont.Color := fOrigFont.Color;
 fColor := fOrigColor;
 Invalidate;
end;

procedure TTelepitoGomb.SetFontActive(Value: TColor);
begin
 if fFontActive <> Value then
  fFontActive := Value;
end;

procedure TTelepitoGomb.SetColorActive(Value: TColor);
begin
