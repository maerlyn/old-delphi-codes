...
#	ifdef USE_STRM_OUTPUT
	if ( pG->redirect_data ) return ( pG->redirect_size != pG->lrec.ucsize );
#	endif
	if ( !stat( pG->filename, &pG->statbuf ) ) {
		...
		if ( pG->vflag ) printf( "Deleted pre-existing file %s", pG->filename );
	}
#  ifdef NOVELL_BUG_FAILSAFE
	if ( pG->dne && ((pG->outfile = fopen( pG->filename, FOPR )) != NULL) ) {
		printf( "File %s open failed due to Novell Bug", pG->filename );
		fclose( pG->outfile );
		return 1;   /* with "./" fix in checkdir(), should never reach here */
	}
#  endif
	if ( (pG->outfile = fopen( pG->filename, FOPW )) == NULL ) {
		printf( "Error opening file %s for write", pG->filename );
		return 1;
	}
	SHChangeNotify( SHCNE_CREATE, SHCNF_PATH, GetFullPath( pG, pG->filename ), NULL );
	Trace( (pG, "open_outfile:  fopen(%s) for writing succeeded\n", pG->filename) );

#  ifdef USE_FWRITE
	setbuf( pG->outfile, NULL );   /* make output unbuffered */
#  endif
	return 0;
...
void undefer_input( struct Globals *pG ) {
	if ( pG->incnt > 0 ) pG->csize += pG->incnt;
...
int readbyte( struct Globals *pG ) {
	if ( pG->mem_mode ) return EOF;
	if ( pG->csize <= 0 ) {
		pG->csize--;             /* for tests done after exploding */
		pG->incnt = 0;
		return EOF;
	}
	if ( pG->incnt <= 0 ) {
		if ( (pG->incnt = vclRead( pG, (char *)pG->inbuf, INBUFSIZ )) == 0 ) {
			pG->incnt = 0;       /* do not allow negative value to affect stuff */
			return EOF;
		} else if ( pG->incnt < 0 ) {  /* "fail" (abort, retry, ...) returns this */
			printf( "Error reading from ZIP file" );
			EXIT( UEN_READ01 );  /* (PK_BADERR) totally bailing; better than lock-up */
		}
		pG->cur_zipfile_bufstart += INBUFSIZ; /* always starts on block bndry */
		pG->inptr = pG->inbuf;
		defer_leftover_input( pG );           /* decrements pG->csize */
	}
	...
	if ( pG->pInfo->encrypted ) {
		 ...
		 */
		for ( n = pG->incnt, p = pG->inptr;  n--;  p++ ) zdecode( *p );
    }
#endif /* CRYPT */
...
#	ifdef USE_STRM_OUTPUT
	...
	if ( ProgrSize ) user_callback( pG, 2, 0, ProgrSize, NULL ); // bump up progress bar
#  else
	if ( size ) user_callback( pG, 2, 0, size, NULL );
#	endif
	...
	if ( pG->tflag || !size )			 /* testing or nothing to write:  all done */
		return 0;
	if ( pG->disk_full ) return 50;	 /* disk already full:  ignore rest of file */
	/*---------------------------------------------------------------------------
	 ...
	 *---------------------------------------------------------------------------*/
	if ( !pG->pInfo->textmode ) {   /* write raw binary data */
...
#		endif
		if ( !pG->cflag && WriteError( rawbuf, size, pG->outfile ) ) return disk_error( pG );
		...
		if ( pG->newfile ) {
			pG->didCRlast = false;   /* no previous buffers written */
			pG->newfile   = false;
		}
		p = rawbuf;
		if ( *p == LF && pG->didCRlast ) ++p;
...
time_t dos_to_unix_time( unsigned ddate, unsigned dtime ) {
	 ...
	 *-------------------------------------------------------------------------*/
	{
		...
		if (res != TIME_ZONE_ID_UNKNOWN) m_time += (60 * tzinfo.Bias);
	}
	/*---------------------------------------------------------------------------
...
int check_for_newer( struct Globals *pG, char *filename ) {
	...
	if ( stat( filename, &pG->statbuf ) ) {
		...
		filename, stat( filename, &pG->statbuf )) );
	 ...
	 */
	if ( pG->extra_field && ef_scan_for_izux(pG->extra_field, pG->lrec.extra_field_length,
			&z_utime, NULL) > 0 ) {
		...
		existing = pG->statbuf.st_mtime & 1 ? pG->statbuf.st_mtime + 1
                                          : pG->statbuf.st_mtime;
		archive  = dos_to_unix_time( pG->lrec.last_mod_file_date,
		                                    pG->lrec.last_mod_file_time );
	}
#else /* !USE_EF_UX_TIME */
	/* round up existing filetime to nearest 2 seconds for comparison */
 ...
 * Return PK-type error code.
	len    :: Without prototype, ush converted to this.
	...
	switch ( option ) {
		...
		*/
		case DISPLAY:
			...
			while ( comment_bytes_left > 0 && block_length > 0 ) {
				...
				if ( (block_length = readbuf( pG, (char *)pG->outbuf,
						(unsigned)MIN( (long)OUTBUFSIZ, comment_bytes_left ))) == 0 ) return PK_EOF;
				...
				*q = '\0';
				...
				A_TO_N( pG->outbuf );   /* translate string to native */
		...
		case DS_FN:
			extra_len = 0;
			if ( len >= FILNAMSIZ ) {
				printf( "Warning - filename too long; truncated" );
				error = PK_WARN;
				extra_len = (ush)(len - FILNAMSIZ + 1);
				len = FILNAMSIZ - 1;
			}
			if ( readbuf( pG, pG->filename, len ) == 0 ) return PK_EOF;
			pG->filename[len] = '\0';   /* terminate w/zero:  ASCIIZ */
			...
			A_TO_N( pG->filename );       /* translate string to native      */
			...
			if ( pG->pInfo->vollabel && len > 8 && pG->filename[8] == '.' ) {
				...
				while ( *p++ ) p[-1] = *p;  /* disk label, and 8th char is dot:  remove dot */
			}
...
void ExtASCII2Native( struct Globals *pG, char *string, int hostnum, short int hostver ) {
	if ( pG->WantedCodePage == cpAUTO && (hostnum == FS_FAT_ || hostnum == FS_HPFS_ || (hostnum == FS_NTFS_ && hostver == 50))
...
long vclSeek( struct Globals *pG, long Offset, int FromWhere ) {
	if ( pG->UseInStream ) {
		switch( FromWhere ) {
			case SEEK_CUR:	pG->StreamPos += Offset; break;
			case SEEK_END: pG->StreamPos  = pG->InStreamSize - Offset; break;
			case SEEK_SET: pG->StreamPos  = Offset;
		}
