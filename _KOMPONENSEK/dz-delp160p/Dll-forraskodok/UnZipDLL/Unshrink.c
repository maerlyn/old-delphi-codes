...
#ifdef DEBUG
#  define OUTDBG(c) \
   if ((c) < 32 || (c) >= 127) printf( "\\x%02x", (c) ); else putc( (c), stderr );
#else
#  define OUTDBG( c )
#endif
 ...
 *									Function unshrink()
 */
int unshrink( struct Globals *pG ) {
#if 0
	...
	if ( pG->pInfo->textmode && !pG->outbuf2 &&
			(pG->outbuf2 = (uch *)MALLOC( TRANSBUFSIZ )) == (uch *)NULL ) return PK_MEM3;
...
#	endif
	...
	do {
		...
		while ( parent[freecode] != FREE_CODE ) ++freecode;
