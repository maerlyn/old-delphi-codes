 ...
 *									Function find_ecrec()
 */
static int find_ecrec( struct Globals *pG, long searchlen ) {	/* return PK-class error */
...
fail:
	...
	if ( readbuf( pG, (char *)byterec, ECREC_SIZE + 4 ) == 0 ) return PK_EOF;
	...
	pG->expect_ecrec_offset = pG->ecrec.offset_start_central_directory +
			pG->ecrec.size_central_directory;
	return PK_COOL;
...
int uz_end_central( struct Globals *pG ) {
	...
	if ( pG->ecrec.zipfile_comment_length && (pG->zflag > 0 || ( pG->zflag == 0 &&
		...
		if ( do_string( pG, pG->ecrec.zipfile_comment_length, DISPLAY ) ) {
			printf( "zip file comment too long" );
			error = PK_WARN;
		}
	}
	return error;
...
int process_cdir_file_hdr( struct Globals *pG ) {
	...
	if ( pG->L_flag )             /* if user specified case-conversion */
		switch ( pG->pInfo->hostnum ) {
			...
			case VMS_:			/* our Zip uses lowercase, but asi's doesn't */
				pG->pInfo->lcflag = 1;   /* convert filename to lowercase   */
				break;
	...
	if ( IS_VOLID( pG->crec.external_file_attributes ) &&
			(pG->pInfo->hostnum == FS_FAT_  || pG->pInfo->hostnum == FS_HPFS_ ||
			 pG->pInfo->hostnum == FS_NTFS_ || pG->pInfo->hostnum == ATARI_) ) {
		pG->pInfo->vollabel = true;
		pG->pInfo->lcflag   = 0;        /* preserve case of volume labels */
	} else
...
int get_cdir_ent( struct Globals *pG ) {
	...
	pG->crec.external_file_attributes	  = makelong( &byterec[C_EXTERNAL_FILE_ATTRIBUTES] );  /* LONG, not word! */
 ...
 */
int process_local_file_hdr( struct Globals *pG ) {   /* return PK-type error code */
	 ...
	 * usable struct (lrec)).
	 *---------------------------------------------------------------------------*/
	if ( readbuf( pG, (char *)byterec, LREC_SIZE) == 0 ) return PK_EOF;
