...
static int explode_lit8( struct Globals *pG, struct huft *tb, struct huft *tl,
								 struct huft *td, int bb, int bl, int bd );
static int explode_lit4( struct Globals *pG, struct huft *tb, struct huft *tl,
								 struct huft *td, int bb, int bl, int bd );
static int explode_nolit8( struct Globals *pG, struct huft *tl,
									struct huft *td, int bl, int bd );
static int explode_nolit4( struct Globals *pG, struct huft *tl,
									struct huft *td, int bl, int bd );
...
static int explode_lit8( struct Globals *pG, struct huft *tb, struct huft *tl, struct huft *td, int bb, int bl, int bd ) {
	...
	while ( s > 0 ) {             /* do until ucsize bytes uncompressed */
		...
		if ( b & 1 ) {             /* then literal--decode it */
			...
			if ( (e = (t = tb + ((~(unsigned)b) & mb))->e) > 16 ) do {
				if ( e == 99 ) return 1;
				DUMPBITS( t->b )
				e -= 16;
				NEEDBITS( e )
...
#	endif
	...
	pG->hufts = 0;                    /* initialize huft's malloc'ed  */
	if ( pG->lrec.general_purpose_bit_flag & 4 ) {
		bb = 9;                        /* base table size for literals */
		if ( (r = get_tree( pG, l, 256 )) != 0 ) return (int)r;
		if ( (r = huft_build( pG, l, 256, 256, NULL, NULL, &tb, &bb )) != 0 ) {
			if ( r == 1 ) huft_free( tb );
			return (int)r;
		}
		if ( (r = get_tree( pG, l, 64 )) != 0 ) return (int)r;
