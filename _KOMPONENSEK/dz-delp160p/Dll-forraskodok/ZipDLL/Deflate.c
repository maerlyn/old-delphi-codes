...
/* Compile with HASH_BITS = 14 to reduce the memory requirements or
 * with HASH_BITS = 13 to use as little memory as possible. Otherwise if the
 * entire input file can be held in memory (not possible on 16 bit systems).
 * Warning: defining this symbol affects the compression ratio.
 * The compressed output is still correct, and might even be smaller in some cases.
...
int  longest_match( IPos cur_match, struct Globals *pG );
...
#define INSERT_STRING( s, match_head ) \
   (UPDATE_HASH( pG->ins_h, pG->window[(s) + (MIN_MATCH - 1)] ), \
    pG->prev[(s) & WMASK] = (ush)(match_head = pG->head[pG->ins_h]), \
    pG->head[pG->ins_h] = (ush)(s))	/* RCV Added 2x (ush) */
...
void lm_init( int pack_level, ush *flags, struct Globals *pG ) {
	...
	if ( pack_level < 1 || pack_level > 9 ) ziperr( ZEN_LOGIC03, pG );
	...
	if ( pG->window == NULL ) {
		...
		if ( pG->window == NULL ) ziperr( ZEN_MEM02, pG );
	}
	if ( pG->prev == NULL ) {
		pG->prev   = (Pos *)zcalloc( WSIZE,     sizeof( Pos ) );
		pG->head   = (Pos *)zcalloc( HASH_SIZE, sizeof( Pos ) );
		if ( pG->prev == NULL || pG->head == NULL ) ziperr( ZEN_MEM03, pG );
	}
#endif /* DYN_ALLOC */
	 ...
	 */
	if ( pG->lookahead < MIN_LOOKAHEAD ) fill_window( pG );

	pG->ins_h = 0;
	for( j = 0; j < MIN_MATCH - 1; j++ ) UPDATE_HASH( pG->ins_h, pG->window[j] );
 ...
 * match.S. The code is functionally equivalent, so you can use the
 * C version if desired.
...
int longest_match( IPos cur_match, struct Globals *pG ) {
	...
	if ( pG->prev_length >= pG->good_match ) chain_length >>= 2;
...
static void check_match( IPos start, IPos match, int length, struct Globals *pG ) {
	...
	if ( memcmp( (char*)pG->window + match, (char*)pG->window + start, length) != EQUAL ) {
		printf( " start %d, match %d, length %d\n", start, match, length );
		error( "invalid match", pG );
	}
}
#else
...
static void fill_window( struct Globals *pG ) {
	...
	do {
		...
		if ( more == (unsigned)EOF ) {
			...
			pG->strstart    -= WSIZE; /* we now have strstart >= MAX_DIST: */
			...
			if ( pG->global_abort_sw ) return;	/* UPDATE: the ziperr() func in dllzip.c closes file. */
		}
		if ( pG->eofile ) return;
...
static ulg deflate_fast( struct Globals *pG ) {
	...
	while ( pG->lookahead != 0 ) {
		...
		if ( hash_head != NIL && pG->strstart - hash_head <= MAX_DIST ) {
			...
			if ( match_length > pG->lookahead ) match_length = pG->lookahead;
		}
		...
		if ( flush ) FLUSH_BLOCK( 0 ), pG->block_start = pG->strstart;
...
ulg deflate( struct Globals *pG ) {
	...
	if ( pG->level <= 3 ) return deflate_fast( pG ); /* optimized for speed */
	...
	while ( pG->lookahead != 0 ) {
		...
		if ( hash_head != NIL && pG->prev_length < pG->max_lazy_match &&
			...
			if ( match_length == MIN_MATCH && pG->strstart - pG->match_start > TOO_FAR ) {
#endif
	...
	if ( match_available ) ct_tally( 0, pG->window[pG->strstart - 1], pG );
