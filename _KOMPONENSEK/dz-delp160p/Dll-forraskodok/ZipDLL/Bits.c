...
static void flush_outbuf( unsigned w, unsigned bytes, struct Globals *pG );
...
static void flush_outbuf( unsigned w, unsigned bytes, struct Globals *pG ) {
	...
	if ( bytes == 2 ) {
		PUTSHORT( w );
	} else if ( bytes == 1 ) pG->out_buf[pG->out_offset++] = (char)(w & 0xFF);
}
...
void bi_windup( struct Globals *pG ) {
	...
	if ( pG->zfile != NULL ) flush_outbuf( 0, 0, pG );
	pG->bi_buf   = 0;
	pG->bi_valid = 0;
#ifdef DEBUG
...
void copy_block( char *block, unsigned len, int header, struct Globals *pG ) {
	...
	if ( pG->zfile ) {
		...
		if ( ferror( pG->zfile ) ) ziperr( ZEN_WRITE02, pG );
 ...
 * the first six bytes (method and crc).
	*tgt, *src		   :: Target and source buffers.
	 tgtsize, srcsize :: Target and source sizes.
 */
ulg memcompress( char *tgt, ulg tgtsize, char *src, ulg srcsize, struct Globals *pG ) {
	...
	if ( tgtsize <= 6L ) ziperr( ZEN_LOGIC02, pG );
