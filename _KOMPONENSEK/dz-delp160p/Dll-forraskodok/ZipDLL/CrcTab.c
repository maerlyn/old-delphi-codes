...
#  if 0          /* not used, since sizeof("near *") <= sizeof(int) */
	/* Use this section when access to a "static int" is faster than access to
    * a "static pointer" (e.g.: i86 16bit code with far pointers). */
...
# ifdef COMPUTE_XOR_PATTERN
	...
	xor = 0L;
	for ( i = 0; i < sizeof( p ) / sizeof( uch ); i++ ) xor |= 1L << (31 - p[i]);
# else
...
# ifdef DYNALLOC_CRCTAB
	...
	if ( crctab_p == NULL ) ziperr( ZEN_MEM01 );
# endif /* DYNALLOC_CRCTAB */

	for ( n = 0; n < 256; n++ ) {
		c = (ulg)n;
		for ( k = 8; k; k-- ) c = c & 1 ? xor ^ (c >> 1) : c >> 1;
		crctab_p[n] = c;
	}
	MARK_CRCTAB_FILLED;
}
