...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, ComCtrls, Db, Grids, DBGrids,
  SRColBtn, IniFiles, ButtonWithColor, dbf;
...
begin
 ...
 begin
  if cbxKolcsonkeroNev.Checked then
   if Pos(LowerCase(txtKolcsonkeroNev.Text),LowerCase(ADOTable1.FieldByName('Nev').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxKolcsonkeroCim.Checked then
   if Pos(LowerCase(txtKolcsonkeroCim.Text),LowerCase(ADOTable1.FieldByName('Cim').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxKolcsonkeroTelszam.Checked then
   if Pos(LowerCase(txtKolcsonkeroTelszam.Text),LowerCase(ADOTable1.FieldByName('Telszam').AsString)) > 0 then
    Inc(HanyFeltetelOK);
...
begin
 ...
 begin
  if cbxCDNev.Checked then
   if Pos(LowerCase(txtCDNev.Text),LowerCase(ADOTable1.FieldByName('CDNeve').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxCDTartalom.Checked then
   if Pos(LowerCase(txtCDTartalom.Text),LowerCase(ADOTable1.FieldByName('Kategoria').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxCDPontosTartalom.Checked then
   if Pos(LowerCase(txtCDPontosTartalom.Text),LowerCase(ADOTable1.FieldByName('Tartalom').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxCDHely.Checked then
   if Pos(LowerCase(txtCDHely.Text),LowerCase(ADOTable1.FieldByName('Hely').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxCDMegjegyzes.Checked then
   if Pos(LowerCase(txtCDMegjegyzes.Text),LowerCase(ADOTable1.FieldByName('Megjegyzes').AsString)) > 0 then
    Inc(HanyFeltetelOK);
  if cbxCDKolcsonVanEKerve.Checked then
   if cbxCDKolcsonVanEKerveErtek.Checked = ADOTable1.FieldByName('KolcsonVan').AsBoolean then
    Inc(HanyFeltetelOK);
  if cbxCDKolcsonkeresDatuma.Checked then
   if int(dtpCDKolcsonkeresDatuma.Date) = int(ADOTable1.FieldByName('Datum').AsDateTime) then
    Inc(HanyFeltetelOK);
  if cbxCDKolcsonkero.Checked then
   if Pos(LowerCase(txtCDKolcsonkero.Text),LowerCase(ADOTable1.FieldByName('Kolcsonker').AsString)) > 0 then
    Inc(HanyFeltetelOK);

  if HanyFeltetelVan = HanyFeltetelOK then
   ListBox1.Items.Add(ADOTable1.FieldByName('CDNeve').AsString);
  HanyFeltetelOK := 0;
  ADOTable1.Next;
  Application.ProcessMessages;
  Timer1Timer(ADOTable1);
...
var MireKattintottSzam: integer;
    ...
    CDkf: TfrmCDk;
begin
 MireKattintottSzam := 0;

 for i := 0 to ListBox1.Items.Count - 1 do
  if ListBox1.Selected[i] then
   MireKattintottSzam := i;
