...
uses
  ...
  StdCtrls, Db, IniFiles, ExtCtrls, Printers, ButtonWithColor, dbf;
...
begin
 ...
 begin
  if ADOTable1.FieldByName('KolcsonVan').AsBoolean then
   lstKolcsonadottCDk.Items.Add(ADOTable1.FieldByName('CDNeve').AsString);
  ADOTable1.Next;
...
var i, NapokSzama, ForintPerNap: integer;
    CDNeve, temp: string;
    Cfg: TIniFile;
    TempDate: TDate;
    tmp: string;
...
begin
 ...
 ADOTable1.Edit;
 ADOTable1.FieldByName('KolcsonVan').AsBoolean := false;
 ADOTable1.Post;
 ...
 Cfg.Free;
 TempDate := ADOTable1.FieldByName('Datum').AsDateTime;
 TempDate := int(TempDate + NapokSzama);
// ADOTable1.Close;
 if TempDate <= Now then
 begin
  CDNeve := ADOTable1.FieldByName('CDNeve').AsString;
  temp := 'A ' + ADOTable1.FieldByName('CDNeve').AsString + ' CD ' + IntToStr(NapokSzama);
  temp := temp + ' napnál tovább volt ' + NalNel(ADOTable1.FieldByName('Kolcsonker').AsString) + ' ';
  KolcsonkeroNeve := ADOTable1.FieldByName('Kolcsonker').AsString;
...
//    Image1.Picture.Bitmap.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'szamla.bmp');
    ...
    ADOTable1.Open;
    ADOTable1.Locate('Nev',KolcsonkeroNeve,[]);
    KolcsonkeroCime := ADOTable1.FieldByName('Cim').AsString;
    ADOTable1.Close;
...
// ADOTable1.Open;
 ...
 if not (ADOTable1.TableName = 'CDk') then
 begin
  if ADOTable1.Active then
   ADOTable1.Close;
...
var CDNeve: string;
    ...
    Cfg: TIniFile;
begin
 for i := 0 to lstKolcsonadottCDk.Items.Count - 1 do
  if lstKolcsonadottCDk.Selected[i] then
   CDNeve := lstKolcsonadottCDk.Items[i];
 ADOTable1.Locate('CDNeve',CDNeve,[]);
 txtKolcsonkero.Text := ADOTable1.FieldByName('Kolcsonker').AsString;
 txtKolcsonadasDatuma.Text := DateToStr(ADOTable1.FieldByName('Datum').AsDateTime);
 txtHanyNapjaVanNala.Text := IntToStr(trunc(Now)-trunc(ADOTable1.FieldByName('Datum').AsDateTime));
 Cfg := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'CDNyilvantarto.ini');
...
procedure TfrmCDVisszakapasa.FormActivate(Sender: TObject);
begin
 ADOTable1.Open;
 ADOTable1.First;
 lstKolcsonadottCDk.Items.Clear;
