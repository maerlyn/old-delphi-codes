...
uses
  ...
  ShellAPI, ActnList, Menus, IniFiles, Beallitasok, Kereses,
  About, Grids, ImgList, CDNyDataFile, Db;
...
{------------------------------------------------------------------------------}
 ...
 begin
  ...
  begin
   if ADOTable1.GetIndex(i).KolcsonVan then
    begin
    TempDate := ADOTable1.GetIndex(i).Datum;
    ...
    begin
     ...
     temp := temp + NalNel(ADOTable1.GetIndex(i).Kolcsonker) + '(';
...
begin
 ...
 begin
  ...
  StringGrid1.Font.Color := fBetuk;
  StringGrid1.Canvas.Font.Color := fBetuk;
 end;
end;
