...
uses
  ...
  StdCtrls, DBCtrls, Mask, ComCtrls, ExtCtrls, MainForm, IniFiles,
  ButtonWithColor, CDNyDataFile;
...
begin
 ...
 for i := 1 to CDNyDataFile_CDk1.Mennyiseg do
  if CDNyDataFile_CDk1.GetIndex(i).CDNeve = cmbCDNeve.Text then
   j := i;
 if j = -1 then
 begin
 ...
 txtPontosTartalom.Lines.Text := CDNyDataFile_CDk1.GetPT(j);
...
begin
 ...
 if j <> -1 then
 begin
  CDNyDataFile_CDk1.SetIndex(j,cKategoria,cmbTartalom.Text);
  CDNyDataFile_CDk1.SetPT(j,txtPontosTartalom.Lines.Text);
  CDNyDataFile_CDk1.SetIndex(j,cHely,txtHely.Text);
  CDNyDataFile_CDk1.SetIndex(j,cMegjegyzes,txtMegjegyzes.Text);
  CDNyDataFile_CDk1.SetIndex(j,cKolcsonVanEKerve,cbxKolcsonVanEKerve.Checked);
  CDNyDataFile_CDk1.SetIndex(j,cKolcsonkeresDatuma,dtpKolcsonadasDatuma.Date);
  CDNyDataFile_CDk1.SetIndex(j,cKolcsonkero,cmbKolcsonkero.Text);
 end
 else
 begin
  j := CDNyDataFile_CDk1.Mennyiseg + 1;
  CDNyDataFile_CDk1.Mennyiseg := j;
  CDNyDataFile_CDk1.SetIndex(j,cKategoria,cmbTartalom.Text);
  CDNyDataFile_CDk1.SetPT(j,txtPontosTartalom.Text);
  CDNyDataFile_CDk1.SetIndex(j,cHely,txtHely.Text);
  CDNyDataFile_CDk1.SetIndex(j,cMegjegyzes,txtMegjegyzes.Text);
  CDNyDataFile_CDk1.SetIndex(j,cKolcsonVanEKerve,cbxKolcsonVanEKerve.Checked);
  CDNyDataFile_CDk1.SetIndex(j,cKolcsonkeresDatuma,dtpKolcsonadasDatuma.Date);
...
begin
 ...
 CDNyDataFile_CDk1.Mennyiseg := CDNyDataFile_CDk1.Mennyiseg - 1;
...
begin
 ...
 for i := 1 to CDNyDataFile_CDk1.Mennyiseg do
  cmbCDNeve.Items.Add(CDNyDataFile_CDk1.GetIndex(i).CDNeve);

 cmbKolcsonkero.Items.Clear;
 for i := 1 to CDNyDataFile_Kolcsonkerok1.Mennyiseg do
  cmbKolcsonkero.Items.Add(CDNyDataFile_Kolcsonkerok1.GetIndex(i).Nev);
end;
