...
uses
  ...
  ImgList, Menus, ActnList, CDNyDataFile2, IniFiles, Beallitasok,
  Kereses, About, Toltes, ShellAPI;
...
type
  TfrmMainForm = class(TForm)
    ...
    procedure cmdKolcsonkapasClick(Sender: TObject);
  private
...
uses Kolcsonkerok, CDk, KolcsonkapottCDk;
...
{------------------------------------------------------------------------------}
 ...
 begin
  ...
  begin
   ...
   if (tKep.Width = 32)and(tKep.Height = 32) then
    cmdKolcsonkapas.Glyph.LoadFromFile(Konyvtar + '5.bmp');
  end;
...
{------------------------------------------------------------------------------}
 ...
 begin
  ...
  frmToltes.Show;
  ADOTable1.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'CDk.cdny',frmToltes.Gauge1);
  frmToltes.Hide;
  for i := 1 to ADOTable1.Count do
  begin
...
begin
 if not FileExists(ExtractFilePath(ParamStr(0)) + 'CDNyilvantarto.chm') then
 begin
  Application.MessageBox('Nem találom a súgófilet!','Hiba',mb_Ok + mb_IconHand);
  Exit;
 end;
 ShellExecute(Handle,'open',PChar(ExtractFilePath(ParamStr(0)) + 'CDNyilvantarto.chm'),'',PChar(ExtractFilePath(ParamStr(0))),sw_ShowNormal);
end;
...
procedure TfrmMainForm.cmdKolcsonkapasClick(Sender: TObject);
begin
 if not Assigned(frmKolcsonkapottCDk) then
  frmKolcsonkapottCDk := TfrmKolcsonkapottCDk.Create(Application);
 frmKolcsonkapottCDk.Show;
// frmKolcsonkapottCDk.LoadData;
end;
